        ...
        unsafe extern "C" fn thread_start(main: usize) -> ! {
            {
                let init = Box::from_raw(
                    core::ptr::with_exposed_provenance::<Box<ThreadInit>>(main).cast_mut(),
                );
